...
class Inception(nn.Module):
    def __init__(self, in_channels, n_filters, kernel_sizes=[9, 19, 39], bottleneck_channels=32,
        ...
        """
        Inception PyTorch module.

        :param in_channels: Number of input channels (input features).
        :param n_filters: Number of filters per convolution layer => out_channels = 4*n_filters.
        :param kernel_sizes: List of kernel sizes for each convolution. Each kernel size must be 
                             an odd number that meets -> "kernel_size % 2 !=0". This is necessary because 
                             of padding size. For correction of kernel_sizes use function "correct_sizes".
        :param bottleneck_channels: Number of output channels in the bottleneck. Bottleneck will not be 
                                     used if the number of in_channels is equal to 1.
...
class InceptionBlock(nn.Module):
    def __init__(self, in_channels, n_filters=32, kernel_sizes=[9, 19, 39],
                 bottleneck_channels=32, use_residual=True, activation=nn.ReLU(), return_indices=False):
...
class InceptionTranspose(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes=[9, 19, 39],
                 bottleneck_channels=32, activation=nn.ReLU()):
        """
        : param in_channels                Number of input channels (input features)
        : param n_filters                Number of filters per convolution layer => out_channels = 4*n_filters
...
class InceptionTransposeBlock(nn.Module):
    def __init__(self, in_channels, out_channels=32, kernel_sizes=[9, 19, 39], bottleneck_channels=32, use_residual=True, activation=nn.ReLU()):
        ...
        Parameters:
            ...
            kernel_sizes: a list of kernel sizes for each Inception module branch, default = [9, 19, 39]
            bottleneck_channels: number of channels at the bottleneck, default = 32
            use_residual: a flag indicating whether to use a residual connection, default = True
            activation: an activation function to be applied to the output of the block, default = nn.ReLU()

            Output:
            The output Z of the block after performing the forward pass.
        '''
        self.use_residual = use_residual
        self.activation = activation
